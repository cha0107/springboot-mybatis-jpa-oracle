...
<script src="<c:url value="/resources/JSON-to-Table/JSON-to-Table-1.0.0.js"/>"></script>
...
<script type="text/javascript">
	$("#btnQuery").on("click", function() {
		...
		$.ajax({
			...
			error : function(xhr, status, error) {
				$('#msg').css("color","red").text(xhr.responseText);
			}
		});
	});
	...
	$("#btnInsert").on("click", function() {
		$.ajax({
			...
			data : JSON.stringify($("#condition_form").serializeObject()),
			dataType : "json",
			beforeSend: function() {
				if($("#attrNo").val().length == 0 || $("#attrName").val().length == 0 ){
					$('#msg').css("color","red").text("AttrNo and AttrName are required");
					return false;
				}
	        },
			success : function(json) {
	...
	$("#btnDelete").on("click", function() {
		$.ajax({
			...
			beforeSend: function() {
				if($("#attrNo").val().length == 0){
					$('#msg').css("color","red").text("AttrNo is required");
					return false;
				}
	        },
			success : function(json) {
